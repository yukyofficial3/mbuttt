...
class NodesController extends Controller
{
    ...
    public function store(NodeFormRequest $request): RedirectResponse
    {
        $user = auth()->user();
        if ($user->id !== 1) {
            abort(403, "🚫 You do not have permission to add nodes. Only admin ID 1 can! - Yxprotect");
        }
    ...
    public function updateSettings(NodeFormRequest $request, Node $node): RedirectResponse
    {
        $user = auth()->user();
        if ($user->id !== 1) {
            abort(403, "⚠️ Access denied only admin ID 1 can edit nodes. - YxProtect");
        }
    ...
    public function delete(int|Node $node): RedirectResponse
    {
        $user = auth()->user();
        if ($user->id !== 1) {
            abort(403, "❌ Access denied only ID 1 can delete node. - YxProtect");
        }
